    ...
    def process(self) -> Dict[str, Any]:
        ...
        try:
            with error_handler(self.logger, "formatted_documents_processing"):
                ...
                if not keys:
                    raise RuntimeError(
                        f"No documents under s3://{self.src_bucket}/{self.src_prefix}"
                    )

                self.logger.info(
                    f"Found {len(keys)} document file(s) under s3://{self.src_bucket}/{self.src_prefix}"
                )
                ...
                for idx, key in enumerate(keys, 1):
                    ...
                    self.logger.info(
                        f"[MERGE] ({idx}/{len(keys)}) {key} [tag={tag}] -> records processed: {relevant}"
                    )
                ...
                if not self.dry_run:
                    ...
                    self.logger.info(
                        f"[OK] Wrote JSONL sample to s3://{self.out_bucket}/{self.out_key}"
                    )
