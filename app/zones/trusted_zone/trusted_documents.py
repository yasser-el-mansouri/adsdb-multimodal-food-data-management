...
class TrustedDocumentsProcessor:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.nutrition_per_ingredient_candidates = nutrition_config.get(
            "per_ingredient_key_candidates", []
        )
        self.nutrition_numeric_fields = nutrition_config.get("numeric_fields", [])
        self.drop_totals_if_per_ingredient_present = nutrition_config.get(
        ...
        self.doc_stats = {
            ...
            "text_tokens_avg": {
                ...
                "instructions_raw": 0.0,
                "instructions_clean": 0.0,
                "title_raw": 0.0,
                "title_clean": 0.0,
            },
        }
    ...
    def compute_iqr_thresholds(self, recipe_ids_with_images: Set[str]) -> Dict[str, Any]:
        ...
        self.logger.info(
            "Pre-scanning documents to compute IQR thresholds for nutritional fields..."
        )
        ...
        try:
            with error_handler(self.logger, "trusted_documents_processing"):
                ...
                if not recipe_ids_with_images:
                    raise RuntimeError(
                        "No recipe IDs with images found. Cannot proceed with document filtering."
                    )
    ...
    def _process_dry_run(
        self, recipe_ids_with_images: Set[str], iqr_thresholds: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {"total_docs": total, "kept_docs": kept_after_id, "timestamp": utc_timestamp()}
    ...
    def _process_full(
        ...
        try:
            for raw in self.read_jsonl_lines(self.src_bucket, self.src_key):
                ...
                totals_key = self.pick_first_present(rec, self.nutrition_totals_candidates)
                per_ingr_key = self.pick_first_present(
                    rec, self.nutrition_per_ingredient_candidates
                )
                ...
                title_raw = (
                    rec.get("title__from_layer1")
                    or rec.get("title__from_recipes_with_nutritional_info")
                    or rec.get("title")
                    or ""
                )
                ingr_raw = (
                    self.join_text_list(rec.get("ingredients__from_layer1"))
                    or self.join_text_list(
                        rec.get("ingredients__from_recipes_with_nutritional_info")
                    )
                    or self.join_text_list(rec.get("ingredients"))
                    or ""
                )
                instr_raw = (
                    self.join_text_list(rec.get("instructions__from_layer1"))
                    or self.join_text_list(
                        rec.get("instructions__from_recipes_with_nutritional_info")
                    )
                    or self.join_text_list(rec.get("instructions"))
                    or ""
                )
        ...
        self.logger.info(
            f"[OK] Wrote filtered+transformed docs to s3://{self.out_bucket}/{self.out_key}"
        )
        ...
        report = {
            ...
            "filtering_rate": (
                f"{(self.doc_stats['written']/self.doc_stats['total_read']*100):.2f}%"
                if self.doc_stats["total_read"] > 0
                else "0%"
            ),
            "nutrition": {
            ...
            "nutrition_totals_dropped_due_to_duplication": self.doc_stats[
                "nutrition_totals_dropped_due_to_duplication"
            ],
